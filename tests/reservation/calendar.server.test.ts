...
beforeEach(() => {
  ...
  process.env.CDL_CAL_ARATIRI = "aratiri@group.calendar.google.com";
});
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
  delete process.env.CDL_CAL_ARATIRI;
});
...
describe("isRangeAvailable", () => {
  it("true cuando no hay eventos que solapen las noches", async () => {
    ...
    const ok = await isRangeAvailable("aratiri", { from: "2026-07-02", to: "2026-07-05" });
    expect(ok).toBe(true);
  });
  ...
  it("consulta events.list con ventana ±1 día y singleEvents=true", async () => {
    ...
    await isRangeAvailable("aratiri", { from: "2026-07-02", to: "2026-07-05" });
  ...
  it("lanza si falta GOOGLE_SERVICE_ACCOUNT_JSON", async () => {
    delete process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
    stubFetch({ events: () => new Response(JSON.stringify({ items: [] }), { status: 200 }) });
    await expect(isRangeAvailable("aratiri", { from: "2026-07-02", to: "2026-07-05" })).rejects.toThrow();
  });
...
describe("createBookingEvent", () => {
  it("inserta el evento pagado [CONFIRMADA] con paymentId y estado paid", async () => {
    ...
    const res = await createBookingEvent("aratiri", {
      unitName: "Cabaña Aratirí",
      firstName: "Juan",
      lastName: "Pérez",
      email: "juan@test.com",
      phone: "+54 9 11",
    ...
    expect(captured!.summary).toBe("[CONFIRMADA] Cabaña Aratirí — Juan Pérez");
  ...
  it("lanza si el insert falla", async () => {
    ...
    await expect(
      createBookingEvent("aratiri", {
        unitName: "Cabaña Aratirí", firstName: "J", lastName: "P", email: "j@t.com",
...
describe("findBookingEventByCode", () => {
  it("devuelve el eventId cuando existe un evento con ese código", async () => {
    ...
    const found = await findBookingEventByCode("aratiri", "CDL-2026-AB12");
  ...
  it("devuelve null cuando no hay evento con ese código", async () => {
    stubFetch({ events: () => new Response(JSON.stringify({ items: [] }), { status: 200 }) });
    const found = await findBookingEventByCode("aratiri", "CDL-2026-ZZ99");
